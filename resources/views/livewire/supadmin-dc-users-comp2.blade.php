<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <input wire:model.debounce.300ms="search" type="text" placeholder="Search users" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <button wire:click="create" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-150 ease-in-out">
                Add User
            </button>
        </div>
    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @php $system = $users->where('id', '>=', 1)->where('id', '<=', 5); @endphp
                            @forelse($system as $user)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $user->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">{{ $user->status ?: 'N/A' }}</span>
                                    </td>
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @php $teacher = $users->where('teacher_id', '>', 0); @endphp
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @php $requested = $users->filter(function($u){ return (bool)($u->is_requested ?? false); }); @endphp
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @php $student = $users->filter(function($u){ return ($u->studentdb_id > 0) && ($u->teacher_id <= 0) && (bool)!($u->is_requested ?? false); }); @endphp
                ...
                <div class="bg-white shadow-md rounded-lg overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @php $suspendedTeachers = $users->filter(function($u){ return ($u->status === false || $u->status === 'inactive') && ($u->teacher_id > 0); }); @endphp
                ...
                <div class="bg-white shadow-md rounded-lg overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @php $suspendedStudents = $users->filter(function($u){ return ($u->status === false || $u->status === 'inactive') && ($u->studentdb_id > 0); }); @endphp
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @php $unknown = $users->filter(function($u){ return ($u->studentdb_id <= 0) && ($u->teacher_id <= 0) && (bool)!($u->is_requested ?? false); }); @endphp
                            @forelse($unknown as $user)
                                <tr class="hover:bg-slate-50/40">
                                    ...
                                    <td class="px-4 py-2 text-sm text-gray-600">{{ $user->email }}</td>
                                    <td class="px-4 py-2">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $user->is_requested ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-800' }}">{{ $user->is_requested ? 'Requested' : 'No' }}</span>
                                    </td>
                                    <td class="px-4 py-2 text-right">
                                        <div class="flex gap-2 justify-end">
                                            @if($user->id > 5)
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($users->where('teacher_id', '>=', 0) as $user)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $user->role ? $user->role->name : 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $user->teacher ? $user->teacher->name : 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $user->studentdb ? $user->studentdb->first_name . ' ' . $user->studentdb->last_name : 'N/A' }}</td>
                            <td class="px-6 py-4">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $user->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">{{ $user->status ?: 'N/A' }}</span>
                            </td>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">{{ $isEdit ? 'Edit User' : 'Add New User' }}</h3>
                            <div class="mt-4 w-full">
                                <form wire:submit.prevent="store">
                                    <div class="space-y-4">
                                        <div>
                                            <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                                            <input type="text" wire:model.defer="name" id="name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('name') border-red-500 @enderror" placeholder="Enter name">
                                            @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                                            <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                                            <input type="email" wire:model.defer="email" id="email" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('email') border-red-500 @enderror" placeholder="Enter email" {{ $isEdit ? 'readonly' : '' }}>
                                            @error('email') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                                            <label for="password" class="block text-sm font-medium text-gray-700">Password {{ !$isEdit ? '(Required)' : '(Leave blank to keep current password)' }}</label>
                                            <input type="password" wire:model.defer="password" id="password" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('password') border-red-500 @enderror" placeholder="Enter password">
                                            @error('password') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                                            <label for="role_id" class="block text-sm font-medium text-gray-700">Role</label>
                                            <select wire:model.defer="role_id" id="role_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('role_id') border-red-500 @enderror">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model.defer="teacher_id" id="teacher_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                        ...
                                        <div>
                                            <label for="studentdb_id" class="block text-sm font-medium text-gray-700">Student</label>
                                            <select wire:model.defer="studentdb_id" id="studentdb_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model.defer="status" id="status" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
