                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-2xl leading-6 font-bold text-gray-900 mb-4" id="modal-headline">
                                {{ $exam01NameId ? 'Edit Exam Name' : 'Add New Exam Name' }}
                            </h3>
                            <div class="mt-2">
                                <form>
                                    @csrf
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="text" 
                                                   ...
                                                   id="exam_month" 
                                                   class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border @error('exam_month') border-red-500 @enderror"
                                                   placeholder="Enter exam month">
                                            @error('exam_month') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="text" 
                                                   ...
                                                   id="exam_year" 
                                                   class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border @error('exam_year') border-red-500 @enderror"
                                                   placeholder="Enter exam year">
                                            @error('exam_year') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
            ...
            <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 w-full md:w-auto">
                ...
                <div class="flex space-x-2">
                    <select wire:model="perPage" 
                        ...
                        <option value="{{ $exam01Names->total() }}">Show All</option>
                    </select>
        ...
        <div class="overflow-x-auto bg-white border border-gray-200 rounded-lg shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($exam01Names ?? collect([]) as $exam01Name)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam01Name->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $exam01Name->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam01Name->exam_month ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam01Name->exam_year ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam01Name->sessionExam ? $exam01Name->sessionExam->name : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $exam01Name->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $exam01Name->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button wire:click="edit({{ $exam01Name->id }})" 
                                        class="text-indigo-600 hover:text-indigo-900 mr-3">
                                    Edit
                                </button>
                                <button wire:click="delete({{ $exam01Name->id }})" 
        ...
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-between">
            <div class="mb-4 sm:mb-0">
                Showing {{ $exam01Names->firstItem() }} to {{ $exam01Names->lastItem() }} of {{ $exam01Names->total() }} results
            </div>
            <div>
                {{ $exam01Names->links() }}
            </div>
