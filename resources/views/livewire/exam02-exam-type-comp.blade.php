                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-2xl leading-6 font-bold text-gray-900 mb-4" id="modal-headline">
                                {{ $exam02TypeId ? 'Edit Exam Type' : 'Add New Exam Type' }}
                            </h3>
                            <div class="mt-2">
                                <form>
                                    @csrf
    ...
    <div class="bg-white shadow rounded-lg p-6">
            ...
            <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 w-full md:w-auto">
                ...
                <div class="flex space-x-2">
                    <select wire:model="perPage" 
                        ...
                        <option value="{{ $exam02Types->total() }}">Show All</option>
                    </select>
        ...
        <div class="overflow-x-auto bg-white border border-gray-200 rounded-lg shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($exam02Types ?? collect([]) as $exam02Type)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam02Type->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $exam02Type->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam02Type->order_index ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $exam02Type->session ? $exam02Type->session->name : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $exam02Type->is_optional ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800' }}">
                                    {{ $exam02Type->is_optional ? 'Yes' : 'No' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $exam02Type->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $exam02Type->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button wire:click="edit({{ $exam02Type->id }})" 
                                        class="text-indigo-600 hover:text-indigo-900 mr-3">
                                    Edit
                                </button>
                                <button wire:click="delete({{ $exam02Type->id }})" 
        ...
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-between">
            <div class="mb-4 sm:mb-0">
                Showing {{ $exam02Types->firstItem() }} to {{ $exam02Types->lastItem() }} of {{ $exam02Types->total() }} results
            </div>
            <div>
                {{ $exam02Types->links() }}
            </div>
