<div x-data="{ isOpen: @entangle('isOpen') }">
                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-2xl leading-6 font-bold text-gray-900 mb-4" id="modal-headline">
                                {{ $myclassSectionId ? 'Edit Class Section' : 'Add New Class Section' }}
                            </h3>
                            <div class="mt-2">
                                <form>
                                    @csrf
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select wire:model="section_id" 
                                                    id="section_id" 
                                                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md p-2 border @error('section_id') border-red-500 @enderror">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select wire:model="teacher_id" 
                                                    id="teacher_id" 
                                                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md p-2 border @error('teacher_id') border-red-500 @enderror">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="number" 
                                                   wire:model="room_id" 
                                                   id="room_id" 
                                                   class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border @error('room_id') border-red-500 @enderror"
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800 mb-4 md:mb-0">Class Section Management</h2>
        ...
        <div class="overflow-x-auto bg-white border border-gray-200 rounded-lg shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($myclassSections ?? collect([]) as $myclassSection)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $myclassSection->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $myclassSection->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $myclassSection->myclass ? $myclassSection->myclass->name : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $myclassSection->section ? $myclassSection->section->name : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $myclassSection->teacher ? $myclassSection->teacher->name : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $myclassSection->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $myclassSection->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <button wire:click="edit({{ $myclassSection->id }})" 
                                        class="text-indigo-600 hover:text-indigo-900 mr-3">
                                    Edit
                                </button>
                                <button wire:click="delete({{ $myclassSection->id }})" 
                                        onclick="confirm('Are you sure you want to delete this class section?') || event.stopImmediatePropagation()"
        ...
        <!-- Pagination -->
        @if($myclassSections->hasPages())
            <div class="mt-6">
                {{ $myclassSections->links() }}
            </div>
        @endif
    </div>
</div>