    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center">
            ...
            <label for="toggle-edit-section" class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" id="toggle-edit-section" class="sr-only peer" wire:click="toggleEditMode" {{ $isEditMode ? 'checked' : '' }}>
                        ...
                        @foreach($sections as $section)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center border-l">
                                <input type="checkbox" 
                                    ...
                                    {{ $isChecked ? 'checked' : '' }}
                                    {{ $isEditMode ? '' : 'disabled' }}
                                    wire:click="updateMapping({{ $myclass->id }}, {{ $section->id }}, $event.target.checked)"
                                >
                            </td>
                        @endforeach
                    </tr>
