                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select wire:model="subject_type_id" 
                                                    id="subject_type_id" 
                                                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md p-2 border @error('subject_type_id') border-red-500 @enderror">
                                                <option value="">-- Select Subject Type --</option>
                                                @foreach($subjectTypes as $subjectType)
                                                    <option value="{{ $subjectType->id }}">{{ $subjectType->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('subject_type_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($subjects ?? collect([]) as $subject)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subject->subjectType ? $subject->subjectType->name : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subject->order_index ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subject->school ? $subject->school->name : '-' }}</td>
