            ...
            @if($classSections->count() > 0 && $examClassSubjects->count() > 0)
                <div class="space-y-8 p-6">
                    ...
                    @foreach($classSections as $section)
                        <div class="border border-gray-200 rounded-lg @if(!$isValidationPassed) opacity-50 cursor-not-allowed @endif">
                            <div class="bg-gray-50 px-6 py-3 border-b border-gray-200 flex justify-between items-center">
                                <h3 class="text-lg font-medium text-gray-900">Section: {{ $section->section->name ?? 'N/A' }}</h3>
                                <button 
                                    wire:click="saveAllEntries"
                                    class="px-4 py-2 text-sm font-medium text-white bg-@if($isValidationPassed) emerald @else gray @endif-600 border border-@if($isValidationPassed) emerald @else gray @endif-600 rounded-md hover:bg-@if($isValidationPassed) emerald @else gray @endif-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-@if($isValidationPassed) emerald @else gray @endif-500 @if(!$isEditingEnabled || !$isValidationPassed) opacity-50 cursor-not-allowed @endif transition-colors duration-200"
                                    @if(!$isEditingEnabled || !$isValidationPassed) disabled @endif
                                >
                                    @if($isValidationPassed) Save All Entries @else Validation Required @endif
                                </button>
                            </div>
